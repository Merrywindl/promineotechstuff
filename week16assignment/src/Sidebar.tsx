...
export default function Sidebar({ addBlankSlide, isExpanded, setIsExpanded }: SidebarProps) {
  ...
  //Resets inputs to blank after submission
  const resetInputs = () => {
    setNewSlideText("");
    setNewSlideStory("");
    setNewSlideImage("");
        ...
        >
          <form style={{ width: "100%" }}>
            <input
              ...
              value={newSlideText}
              onChange={(e) => setNewSlideText(e.target.value)} //i could have used event.target.value but it was easier to just use the e like i saw researching 
            />
            <input
              className="w-75 m-2 border border-black border-5 p-2 bg-light bg-opacity-25 font-weight-bold"
              type="text"
